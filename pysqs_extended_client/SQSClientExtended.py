...
class SQSClientExtended(object):
	...
	aws_region_name=None, s3_bucket_name=None, delete_from_s3=False):
	...
	def receive_message(self, queue_url, max_number_Of_Messages=1, wait_time_seconds=10):
		...
		"""
		response_opt_queue = self.sqs.receive_message(
		...
		for message in opt_messages:
			...
			if large_pay_load_attribute_value:
				try:
					...
					modified_receipt_handle = SQSExtendedClientConstants.S3_BUCKET_NAME_MARKER.value \
						+ s3_bucket_name + SQSExtendedClientConstants.S3_BUCKET_NAME_MARKER.value + \
						SQSExtendedClientConstants.S3_KEY_MARKER.value + s3_key \
						+ SQSExtendedClientConstants.S3_KEY_MARKER.value + message.get('ReceiptHandle')
	...
	def __delete_message_payload_from_s3(self, receipt_handle):
		try:
			...
			s3_object.delete()
			print('Deleted s3 object https://s3.amazonaws.com/{}/{}'.format(s3_msg_bucket_name, 
			s3_msg_key))
	...
	def __get_orig_receipt_handle(self, receipt_handle):
		return receipt_handle[receipt_handle.rindex(
			SQSExtendedClientConstants.S3_KEY_MARKER.value) + len(
				SQSExtendedClientConstants.S3_KEY_MARKER.value):]

	def __is_s3_receipt_handle(self, receipt_handle):
		return True if SQSExtendedClientConstants.S3_BUCKET_NAME_MARKER.value in receipt_handle \
			and SQSExtendedClientConstants.S3_KEY_MARKER.value in receipt_handle else False
	...
	def _send_queue_message(self, queue_url, message, message_group_id, 
	message_deduplication_id, message_attributes, fifo_queue):
		if fifo_queue:
    ...
    Using None is both safe and conventional in cases where arguments with default value are not passed.
		...
		if large_payload_attribute_value:
			raise ValueError(
				"Message attribute name {} is reserved for use by SQS extended client.".format(
					SQSExtendedClientConstants.RESERVED_ATTRIBUTE_NAME.value))
		...
		if self.always_through_s3 or self.__is_large(str(message), message_attributes):
			...
			message_attributes[SQSExtendedClientConstants.RESERVED_ATTRIBUTE_NAME.value] = {
				'StringValue': str(self.__get_string_size_in_bytes(message)), 
				'DataType': 'Number'}
			return self.sqs.send_message(QueueUrl=queue_url, MessageBody=s3_key_message, 
			MessageAttributes=message_attributes, DelaySeconds=delay_seconds)
